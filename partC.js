...
CONFIG.RTA_SILENCE_MS = 10000;   /* <<SIGN-OFF>> silence-net: after this much no-sound, a short prompt reminds
                                    the participant to keep talking. Set to 10s per Dennis 2026-09-10 (was an
                                    open 15s placeholder). The think-aloud announcement states this value,
                                    derived from here, so message and behavior can never drift. */
  ...
  return {
    ...
    on_load: function(){
      ...
      function apShowAnnouncement(cb){
        const isPrac = (caseObj.role === "Practice");
        const secs = Math.round((CONFIG.RTA_SILENCE_MS||10000)/1000);   // silence-net threshold, in seconds
        // Mechanics paragraph (all cases) + the silence-reminder sentence derived from RTA_SILENCE_MS.
        const mech = CONFIG.RTA_AUTOPAUSE_ANNOUNCE + ' If about ' + secs + ' seconds pass without any sound, a short prompt will remind you to keep talking.';
        // Reasoning reminder (all cases); the practice-only prefix leads it on the practice run.
        const reminder = (isPrac ? CONFIG.RTA_PRACTICE_PREFIX : '') + CONFIG.RTA_REASONING_REMINDER;
        ...
        g.innerHTML='<div style="max-width:620px;line-height:1.7">'
          +'<h2 style="color:#ffd8a8;margin:0 0 14px">Talk through your thinking</h2>'
          +'<p style="margin:0 0 18px;font-size:16px">'+mech+'</p>'
          +'<div style="background:rgba(255,216,168,.12);border:1px solid #ffd8a8;border-radius:10px;padding:14px 18px;margin:0 0 22px;font-size:15px">'+reminder+'</div>'
          +'<button id="rp_announce_go" style="background:#1e8e4e;color:#fff;border:0;border-radius:8px;padding:12px 24px;font:700 15px system-ui;cursor:pointer">'+(isPrac?'Start the practice replay':'Start the replay')+'</button>'
          +'</div>';
        document.body.appendChild(g);
